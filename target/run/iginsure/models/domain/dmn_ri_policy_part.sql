create view "dbt_szammit"."dmn_ri_policy_part__dbt_tmp" as
    with 
...
ri_activity as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_ri_activity" where _valid_to is null
),

ri_section as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_ri_section" where _valid_to is null
),

ri_section_broker as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_ri_section_broker" where _valid_to is null
),

ri_section_broker_security as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_ri_section_broker_security" where _valid_to is null
),

type_of_ri_policy as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_ri_policy" where _valid_to is null
),

stg_contact as (
    select * from "IGI_PROD_DW"."dbt_szammit"."stg_contact"
),

stg_broker_group as (
    select * from "IGI_PROD_DW"."dbt_szammit"."stg_broker_group"
),

type_of_fronting as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_fronting" where _valid_to is null
),
...
type_of_ri_class as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_ri_class" where _valid_to is null
),
