  ...
  if object_id ('"dbt_szammit"."actuarial_claim_movement_clean_nb100__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbt_szammit"."actuarial_claim_movement_clean_nb100__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbt_szammit"."actuarial_claim_movement_clean_nb100__dbt_tmp"','U') is not null
    begin
    drop table "dbt_szammit"."actuarial_claim_movement_clean_nb100__dbt_tmp"
    end


   EXEC('create view [dbt_szammit].[actuarial_claim_movement_clean_nb100__dbt_tmp_temp_view] as
...
from "IGI_PROD_DW"."dbt_szammit"."actuarial_claim_movement_load"
    ');

  CREATE TABLE "dbt_szammit"."actuarial_claim_movement_clean_nb100__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM [dbt_szammit].[actuarial_claim_movement_clean_nb100__dbt_tmp_temp_view])
