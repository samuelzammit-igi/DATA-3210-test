  ...
  if object_id ('"dbt_szammit"."actuarial_earned_premium_load__dbt_tmp_temp_view"','V') is not null
    begin
    drop view "dbt_szammit"."actuarial_earned_premium_load__dbt_tmp_temp_view"
    end


   
    
  if object_id ('"dbt_szammit"."actuarial_earned_premium_load__dbt_tmp"','U') is not null
    begin
    drop table "dbt_szammit"."actuarial_earned_premium_load__dbt_tmp"
    end


   EXEC('create view [dbt_szammit].[actuarial_earned_premium_load__dbt_tmp_temp_view] as
...
select * from aggregates
  ...
  CREATE TABLE "dbt_szammit"."actuarial_earned_premium_load__dbt_tmp"
    WITH(
      DISTRIBUTION = ROUND_ROBIN,
      CLUSTERED COLUMNSTORE INDEX
      )
    AS (SELECT * FROM [dbt_szammit].[actuarial_earned_premium_load__dbt_tmp_temp_view])
