create view "dbt_szammit"."dmn_claim__dbt_tmp" as
    with 
...
type_of_event as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_event" where _valid_to is null
),

type_of_event_type as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_event_type" where _valid_to is null
),

type_of_rag as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_rag" where _valid_to is null
),

zone_country as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_zone_country" where _valid_to is null
),
...
type_of_loss as (
    select * from "IGI_PROD_DW"."dbt_szammit"."scd_type_of_loss" where _valid_to is null
),

stg_entity_instance_states as (
    select * from "IGI_PROD_DW"."dbt_szammit"."stg_entity_instance_states" where entity_type = 'Claim Movement' 
),
...
claim_dates as (
    select * from "IGI_PROD_DW"."dbt_szammit"."stg_claim_dates"
),

linked_claim as (
    select * from "IGI_PROD_DW"."dbt_szammit"."stg_claimgroup_linkedclaims"
),
